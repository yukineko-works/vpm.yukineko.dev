...
## v1.8.0
`2025/11/16`  
- Feat: スイッチモジュールに特定のプラットフォームで項目を非表示にできる機能を追加
  - 詳しくは[スイッチモジュール](../settings/switch#disabled-platforms)のドキュメントをご覧ください。  

## v1.7.0
`2025/10/13`
- Feat: スイッチモジュールのスイッチなどをグローバルにできるように  
  - 「スイッチ」「コライダー」「トランスフォーム」「トリガー」の同期に対応しました  
  - 「スイッチ」「コライダー」「トランスフォーム」はあとから参加したユーザー(LateJoiner)にも値が同期されます  
  - 詳しくは[スイッチモジュール](../settings/switch)の各項目をご覧ください。  
- Fix: コライダースイッチの「デフォルトで有効」がOFFになっている場合に、初期化時に対象のオブジェクト自体が非表示になってしまう問題を修正  

::: warning 重要なおしらせ
ワールド統合メニュー本体をv1.3.1以降に更新してからご利用ください。  
v1.3.0以前の場合、同期が正しく行われない可能性があります。
:::
